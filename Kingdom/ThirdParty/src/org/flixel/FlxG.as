package org.flixel
{
	...
	public class FlxG
	{
		...
		//Internal storage system to prevent graphics from being used repeatedly in memory. Seperate because we don't want this cleared ever.
		static private var _externalCache:Object;
		...
		/**
		 * Checks if the requested key is associated with any BitmapData yet
		 * 
		 * @param	key		The key to check for
		 * @return	Whether the requested key is in the external cache or not
		 */
		static public function checkExternalBitmapCache(key:String):Boolean
		{
			return (_externalCache[key] != undefined) && (_externalCache[key] != null);
		}
		/**
		 * Cache a BitmapData and associate it with a key for later retrieval. This is to be used for external images only, as they are never unloaded.
		 * 
		 * @param	key			The key to associate with the data.
		 * @param	data		The BitmapData to cache.
		 * @param	reverse		Whether to generate a flipped version.
		 * @param	unique		Ensures that the bitmap data uses a new slot in the cache.
		 */
		static public function addExternalBitmap(key:String, data:BitmapData):void
		{
			//If there is no data for this key, generate the requested graphic
			if (!checkExternalBitmapCache(key))
			{
				Cc.log("Caching " + key);
				_externalCache[key] = data;
			}
		}
		/**
		 * Returns the previously cached BitmapData associated with the passed in key, or null if it has not been cached.
		 * 
		 * @param	key		The key to check against
		 * @return	The associated BitmapData, or null of none exists
		 */
		static public function getExternalBitmap(key:String):BitmapData
		{
			var swfURL:String = FlxG.stage.loaderInfo.url;
			swfURL = swfURL.substr(0, swfURL.lastIndexOf("/") + 1)
			var actualKey:String = swfURL + key;
			
			if (_externalCache[actualKey] == undefined || _externalCache[actualKey] == null)
			{
				Cc.error("Request for " + key + " failed as it is not in the cache.");
				return null;
			}
			return _externalCache[actualKey];
		}
		/**
		 * Dumps the external cache's image references.
		 */
		static public function clearExternalBitmapCache():void
		{
			_externalCache = new Object();
		}
