package org.flixel
{
	...
	public class FlxGame extends Sprite
	{
		...
		/**
		 * Allows debug to be disabled even when in debug mode-- this allows an external debugger to be used instead
		 */
		...
		//This makes the mouse hide even after the right click menu has been opened
		...
		protected function onKeyUp(FlashEvent:KeyboardEvent):void
		{
			...
			{
				if ((_debugger != null) &&
					((FlashEvent.keyCode == 192) || (FlashEvent.keyCode == 220) &&
					FlxG.debug) &&
					debuggerEnabled)
				{
		...
		{ 
			...
			if (_state != null)
			{
				//Dispatch an event saying the state is being switched from
				var evtFrom:FlxGameEvent = new FlxGameEvent(FlxGameEvent.STATE_SWITCHFROM, _state);
				dispatchEvent(evtFrom);
				_state.destroy();
			}
			...
			//Dispatch an event saying the state is being switched to
			var evtTo:FlxGameEvent = new FlxGameEvent(FlxGameEvent.STATE_SWITCHTO, _state);
			dispatchEvent(evtTo);
		}
